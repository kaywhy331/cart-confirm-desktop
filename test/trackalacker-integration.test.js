...
test("TrackaLacker ingestion crosses only the authenticated extension-to-loopback capture boundary", () => {
  ...
  assert.match(preload, /startTrackalackerImport[\s\S]*?addTrackalackerMissions[\s\S]*?getTrackalackerPriceHistory[\s\S]*?openTrackalackerSource[\s\S]*?openTrackalackerStore/);
...
test("the crawler is sequential, bounded, retryable, and reports progress before mutating missions", () => {
  ...
  assert.match(content, /MAX_HISTORY_ENTRIES = 50/);
  ...
  const main = fs.readFileSync(path.join(root, "main.js"), "utf8");
  assert.match(main, /TRACKALACKER_CHECKPOINT_PRODUCTS = 5/);
