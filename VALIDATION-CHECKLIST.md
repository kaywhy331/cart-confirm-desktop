...
- [ ] In controlled/mock responses, confirm three unreadable results give only that exact product a non-blocking **retrying** status and a ten-minute rest, after which a readable response clears the degraded state. Confirm this retry chatter does not enter Activity. Confirm a `429`/overload or visible security challenge pauses the retailer, while four distinct transport-failing products within one minute open the shared store cooldown without stopping the other retailer.
