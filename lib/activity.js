...
function notificationDeliveryMode(alertLevel, eventType) {
  if (alertLevel !== "silent") return "desktop";
  // Silent missions still need an operator-visible record when a purchase
  // safety gate stops the workflow. Keep it in Activity without producing an
  // operating-system notification; all ordinary silent milestones already
  // have their own Qualified, Added, or Ordered entry.
  return eventType === "automation-blocked" ? "activity" : "none";
}
...
function createActivityEvent(rawEvent = {}, context = {}) {
  ...
  switch (rawEvent.eventType) {
    ...
    case "cart-reached":
      return {
        ...event,
        message: `Cart page is open with ${label} — be ready to complete the purchase.`
      };
