...
function normalizeRuntimeState(input) {
  ...
  for (const [productId, context] of Object.entries(input.productExecutionContexts || {}).slice(-200)) {
    ...
    const participantProductIds = Array.isArray(context?.participantProductIds)
      ? [...new Set(context.participantProductIds.map((value) => String(value || "").slice(0, 100)).filter(Boolean))].slice(0, MAX_PRODUCTS)
      : [];
    if (
      context
  ...
  for (const [cohortKey, capture] of Object.entries(input.queueCaptures || {}).slice(-100)) {
    ...
    const participantProductIds = Array.isArray(capture?.participantProductIds)
      ? [...new Set(capture.participantProductIds.map((value) => String(value || "").slice(0, 100)).filter(Boolean))].slice(0, MAX_PRODUCTS)
      : [];
  ...
  for (const [retailer, timestamps] of Object.entries(input.quietReadHistory || {})) {
    if (!["target", "walmart"].includes(retailer) || !Array.isArray(timestamps)) continue;
    state.quietReadHistory[retailer] = timestamps
      .map(Number)
      .filter((value) => Number.isFinite(value) && value >= 0)
      .sort((left, right) => left - right)
      .slice(-MAX_QUIET_READ_TIMESTAMPS);
  }
  const normalizationTime = Date.now();
  for (const [productId, timestamp] of Object.entries(input.quietLastStartedAt || {}).slice(-200)) {
    const value = Number(timestamp);
    if (!/^(?:target|walmart):\d{5,20}$/.test(productId) || !Number.isFinite(value) || value < 0) continue;
    state.quietLastStartedAt[productId] = Math.min(value, normalizationTime);
  }
