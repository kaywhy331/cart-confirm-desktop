...
function planSignalRoute(options = {}) {
  ...
  if (!automaticOpeningEnabled || !product.enabled || !product.signalAutoOpen) {
  ...
  if (observedAt > now + 30_000) {
    return Object.freeze({ state: "stale", product, url: "", entry: "none", note: "The signal timestamp is in the future and was not trusted." });
  }
  ...
  return Object.freeze({ state: "pending", product, url, entry, note, signalOrderLimit });
}
