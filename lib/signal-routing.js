...
function firstPartySignalSeller(retailer, value) {
  const seller = String(value || "").replace(/\s+/g, " ").trim();
  if (!seller) return null;
  if (retailer === "amazon") return firstPartyAmazonSeller(seller);
  if (retailer === "walmart") return /^(?:sold(?: and shipped)? by )?walmart(?:\.com)?$/i.test(seller);
  if (retailer === "target") return /^(?:sold(?: and shipped)? by )?target(?:\.com)?$/i.test(seller);
  return null;
}

function signalSellerParty(signal = {}, product = {}) {
  if (typeof signal.firstParty === "boolean") return signal.firstParty;
  return firstPartySignalSeller(product.retailer, signal.seller);
}
...
function planSignalRoute(options = {}) {
  ...
  if (settings.signalsEnabled && (!Number.isFinite(signalPrice) || signalPrice <= 0)) {
    return Object.freeze({
      state: "disabled",
      product,
      url: "",
      ...
      strategyDecision,
      note: "The signal did not include a readable positive price, so it could not authorize a purchase attempt."
    });
  }
  if (
  ...
  const allowThirdPartySeller = strategyDecision.state === "matched"
    && strategyDecision.strategy.allowThirdPartySeller === true;
  const signaledFirstParty = signalSellerParty(signal, product);
  if (
    settings.signalsEnabled
    && signaledFirstParty === false
    && !allowThirdPartySeller
  ) {
    return Object.freeze({
      state: "disabled",
      product,
      ...
      strategyDecision,
      note: "The signal named a third-party seller, but this strategy requires the store's first-party offer."
    });
  }

  const offerBinding = Object.freeze({
    maximumPrice: Math.round(Math.min(product.maxPrice, signalPrice) * 100) / 100,
    seller: String(signal.seller || "").replace(/\s+/g, " ").trim().slice(0, 120),
    firstParty: signaledFirstParty,
    allowThirdPartySeller,
  ...
  // Signals must validate the current buy box before any cart mutation. A
  // signal-supplied Buy Now/Add-to-Cart URL can mutate before the content
  // script sees the live seller, so Signals always enters through the product
  // page. Legacy Autopilot-assisted signal opening retains its direct links.
  const directAllowed = !settings.signalsEnabled && Boolean(settings.automationEnabled) && underCap;
  ...
  if (settings.signalsEnabled) {
    note = `${note} The live price and seller must still match this signal before Add to cart is authorized.`;
  }
  return Object.freeze({ state: "pending", product, url, entry, note, signalOrderLimit, strategyDecision, offerBinding });
}

module.exports = {
  SIGNAL_FRESH_MS,
  entryAllowedForProduct,
  firstPartyAmazonSeller,
  firstPartySignalSeller,
  planSignalRoute,
  positiveOrderLimit,
  signalSellerParty
};
