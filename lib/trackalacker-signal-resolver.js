...
function productSlugFromUrl(value) {
  try {
    ...
    return normalizedProductSlug(decodeURIComponent(
      url.pathname.match(/\/products\/showcase\/([^/]{1,240})(?:\/|$)/i)?.[1] || ""
    ));
  } catch {
    return "";
  }
...
function normalizedRetailerSku(retailerValue, skuValue) {
  const retailer = cleanId(retailerValue, 20).toLowerCase();
  const config = RETAILERS[retailer];
  const sku = retailer === "amazon"
    ? String(skuValue || "").trim().toUpperCase()
    : String(skuValue || "").trim();
  return config?.skuPattern.test(sku) ? Object.freeze({ retailer, sku }) : null;
}
...
function mappingFor(item, store) {
  return Object.freeze({
    ...
    sourceProductSlug: productSlugFromUrl(item.sourceUrl),
...
function buildTrackalackerSignalIndex(state = {}, aliases = []) {
  ...
  const byProductSlugRetailer = new Map();
  const bySkuRetailer = new Map();
  ...
  for (const item of Array.isArray(state.items) ? state.items : []) {
    for (const store of Array.isArray(item?.stores) ? item.stores : []) {
      ...
      if (mapping.sourceProductSlug) {
        addIndex(byProductSlugRetailer, `${mapping.sourceProductSlug}|${mapping.retailer}`, mapping);
      }
      addIndex(bySkuRetailer, `${mapping.sku}|${mapping.retailer}`, mapping);
  ...
  return Object.freeze({
    ...
    canonicalSignal: Object.freeze({
      ...
      keywordText: [
        ...
        parsed.envelope?.notification?.title,
        parsed.envelope?.notification?.body,
        ...(parsed.envelope?.notification?.textElements || [])
      ].filter(Boolean).join("\n"),
      seller: "",
...
function resolveTrackalackerSignal(parsed, index, products = [], hints = {}) {
  if (!parsed || parsed.parseState !== "parsed") {
    return result("unresolved", "none", [], parsed || {}, products);
  }
  const parsedRetailer = cleanId(parsed.retailer, 20).toLowerCase();
  const retailerIdentity = normalizedRetailerSku(hints.sourceRetailer, hints.sourceRetailerSku);
  const listingId = cleanId(hints.listingId, 30);

  // Some TrackaLacker preorder pushes use a generic title that does not name
  // the store. In that case, resolve only an exact store-bearing identity.
  // A product ID, slug, or title alone is not enough because the same followed
  // product may have listings at several retailers.
  if (!parsedRetailer) {
    if (listingId) {
      const candidates = (index.byListing.get(listingId) || []).filter((mapping) => (
        !retailerIdentity
        || mapping.retailer === retailerIdentity.retailer && mapping.sku === retailerIdentity.sku
      ));
      return result(candidates.length ? "matched" : "unresolved", "listing-id", candidates, parsed, products);
    }
    if (retailerIdentity) {
      const candidates = index.bySkuRetailer.get(`${retailerIdentity.sku}|${retailerIdentity.retailer}`) || [];
      return result(candidates.length ? "matched" : "unresolved", "retailer-sku", candidates, parsed, products);
    }
    return result("unresolved", "store-identity-missing", [], parsed, products);
  }
  if (listingId) {
    const candidates = (index.byListing.get(listingId) || []).filter((mapping) => mapping.retailer === parsedRetailer);
  ...
  if (sourceProductId) {
    const candidates = index.bySourceProductRetailer.get(`${sourceProductId}|${parsedRetailer}`) || [];
    if (candidates.length) return result("matched", "source-product-retailer", candidates, parsed, products);
  }
  const sourceProductSlug = normalizedProductSlug(hints.sourceProductSlug);
  if (sourceProductSlug) {
    const candidates = index.byProductSlugRetailer.get(`${sourceProductSlug}|${parsedRetailer}`) || [];
    if (candidates.length) return result("matched", "source-product-slug-retailer", candidates, parsed, products);
  }
  if (retailerIdentity?.retailer === parsedRetailer) {
    const candidates = index.bySkuRetailer.get(`${retailerIdentity.sku}|${retailerIdentity.retailer}`) || [];
    if (candidates.length) return result("matched", "retailer-sku", candidates, parsed, products);
  }
  if (!parsed.normalizedProductName) {
    return result("unresolved", "none", [], parsed, products);
  }
  const titleKey = `${parsed.normalizedProductName}|${parsedRetailer}`;
