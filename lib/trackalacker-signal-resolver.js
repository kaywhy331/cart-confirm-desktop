...
function resolveTrackalackerSignal(parsed, index, products = [], hints = {}) {
  if (!parsed || parsed.parseState !== "parsed" || !parsed.retailer) {
  ...
  if (sourceProductSlug) {
    const candidates = index.byProductSlugRetailer.get(`${sourceProductSlug}|${parsed.retailer}`) || [];
  ...
  if (retailerIdentity?.retailer === parsed.retailer) {
