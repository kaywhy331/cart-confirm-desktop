...
function normalizeSettings(input = {}, existing = {}) {
  ...
  const existingSignalBridgeToken = String(existing.signalBridgeToken || "");
  const suppliedSignalBridgeToken = String(input.signalBridgeToken || "");
  const signalBridgeToken = suppliedSignalBridgeToken || existingSignalBridgeToken || newCompanionToken();
  ...
  const trackalackerSignalStartAtLogin = trackalackerSignalBridgeEnabled && (
    input.trackalackerSignalStartAtLogin !== undefined
      ? Boolean(input.trackalackerSignalStartAtLogin)
      : Boolean(existing.trackalackerSignalStartAtLogin ?? DEFAULT_SETTINGS.trackalackerSignalStartAtLogin)
  );
  ...
  return {
    ...
    trackalackerSignalStartAtLogin,
...
function assertSafeArmedUpdate(current, next) {
  if (
    ...
    throw new Error("Disarm automation (Autopilot or Signals) before changing products, quantities, price caps, or purchase actions.");
  }
}
