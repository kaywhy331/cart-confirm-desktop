...
const { MAX_ITEMS, SHARED_FIELDS, itemIdForProduct } = require("./item-missions");
const { MAX_SIGNAL_STRATEGIES, normalizeSignalStrategies } = require("./signal-strategies");
const {
...
const MAX_TRACKALACKER_SIGNAL_ALIASES = 2_000;
const MIN_SIGNAL_DEDUPE_WINDOW_SECONDS = 30;
const MAX_SIGNAL_DEDUPE_WINDOW_SECONDS = 3_600;
const PRODUCT_IMAGE_HOSTS = Object.freeze({
  target: Object.freeze(["target.com", "target.scene7.com"]),
  walmart: Object.freeze(["walmart.com", "walmartimages.com"]),
  amazon: Object.freeze(["amazon.com", "media-amazon.com", "ssl-images-amazon.com"])
});
const TRACKALACKER_HOSTS = Object.freeze(["trackalacker.com"]);
...
const DEFAULT_PRODUCT = Object.freeze({
  ...
  sourceListingUrl: "",
  expectedPrice: 0,
  expectedPriceConfidence: "",
  expectedPriceObservedAt: "",
  sourcePriceSummary: null,
  enabled: false
});

const DEFAULT_SETTINGS = Object.freeze({
  products: Object.freeze([]),
  checkoutTrust: Object.freeze({}),
  missionGroups: Object.freeze([]),
  automationEnabled: false,
  signalsEnabled: false,
  monitoringPaused: false,
  fastMode: true,
  combinedOrderEnabled: false,
  ...
  trackalackerSignalAliases: Object.freeze([]),
  signalStrategies: Object.freeze([]),
...
function normalizeTrackalackerSignalAliases(input) {
  if (!Array.isArray(input)) return [];
  const aliases = [];
  const seen = new Set();
  for (const candidate of input.slice(0, MAX_TRACKALACKER_SIGNAL_ALIASES)) {
    if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) continue;
    const sourceProductId = String(candidate.sourceProductId || "").trim().slice(0, 30);
    const retailer = String(candidate.retailer || "").trim().toLowerCase();
    const alias = String(candidate.alias || "").replace(/\s+/g, " ").trim().slice(0, 240);
    const key = `${sourceProductId}|${retailer}|${alias.toLocaleLowerCase("en-US")}`;
    if (!/^\d{1,20}$/.test(sourceProductId) || !RETAILERS[retailer] || !alias || seen.has(key)) continue;
    seen.add(key);
    aliases.push({ sourceProductId, retailer, alias });
  }
  return aliases;
}
...
function normalizeAffiliateOpenUrl(value, retailer, sku) {
  ...
  try {
    ...
    throw new Error(`Affiliate product link must be a direct HTTPS link to this store option's exact store and item ID. ${error.message || ""}`.trim());
  }
}

function normalizeProductImageUrl(value, retailer) {
  const raw = String(value || "").trim();
  if (!raw || raw.length > 2_048) return "";
  try {
    const url = new URL(raw);
    const hostname = url.hostname.toLowerCase();
    const allowedHosts = PRODUCT_IMAGE_HOSTS[String(retailer || "").toLowerCase()] || [];
    const allowed = [...allowedHosts, ...TRACKALACKER_HOSTS].some((host) => (
...
function normalizeTrackalackerSourceUrl(value, kind) {
  const raw = String(value || "").trim();
  if (!raw || raw.length > 2_048) return "";
  try {
    const url = new URL(raw);
    ...
    const listingPath = /^\/products\/showcase\/[a-z0-9][a-z0-9-]*\/listings\/\d+\/[a-z0-9][a-z0-9-]*(?:\/)?$/i;
    if (
      url.protocol !== "https:"
      || url.username
      || url.password
      || !["trackalacker.com", "www.trackalacker.com"].includes(hostname)
      || !(kind === "listing" ? listingPath : productPath).test(url.pathname)
    ) return "";
...
function normalizeSourceFields(input = {}) {
  if (String(input.sourceProvider || "").toLowerCase() !== "trackalacker") {
    return {
      sourceProvider: "",
      sourceProductId: "",
      sourceUrl: "",
  ...
  const sourceProductId = cleanText(input.sourceProductId, 30);
  const sourceListingId = cleanText(input.sourceListingId, 30);
  const sourceUrl = normalizeTrackalackerSourceUrl(input.sourceUrl, "product");
  const sourceListingUrl = normalizeTrackalackerSourceUrl(input.sourceListingUrl, "listing");
  if (
    !/^\d{1,20}$/.test(sourceProductId)
    || !/^\d{1,20}$/.test(sourceListingId)
    || !sourceUrl
    || !sourceListingUrl
    || !sourceListingUrl.includes(`/listings/${sourceListingId}/`)
  ) return normalizeSourceFields({});
  const expectedPrice = normalizePrice(input.expectedPrice ?? 0);
  const expectedPriceConfidence = ["history", "product", "unavailable"].includes(input.expectedPriceConfidence)
    ? input.expectedPriceConfidence
    : "unavailable";
  const observedAt = new Date(input.expectedPriceObservedAt || "");
  const sourcePriceSummary = normalizeSourcePriceSummary(input.sourcePriceSummary);
  return {
    ...
    expectedPrice,
    expectedPriceConfidence,
    expectedPriceObservedAt: Number.isNaN(observedAt.getTime()) ? "" : observedAt.toISOString(),
    sourcePriceSummary
  };
}

function normalizeSourcePriceSummary(value) {
  ...
  function count(input) {
    const number = Number(input);
    return Number.isFinite(number) ? Math.max(0, Math.min(50, Math.floor(number))) : 0;
  }
  ...
  const latestPrice = price(value.latestPrice);
  if (!sampleCount || latestPrice === null) return null;
  const change = value.changeAmount === null || value.changeAmount === undefined || value.changeAmount === ""
    ? null
    : Number(value.changeAmount);
  return {
    sampleCount,
    trustedSamples: Math.min(sampleCount, count(value.trustedSamples)),
    surgeSamples: Math.min(sampleCount, count(value.surgeSamples)),
    aboveSamples: Math.min(sampleCount, count(value.aboveSamples)),
    latestPrice,
    latestObservedAt: timestamp(value.latestObservedAt),
    latestPriceChangedAt: timestamp(value.latestPriceChangedAt),
    latestClassification: ["normal", "surge", "above", "unknown"].includes(value.latestClassification)
      ? value.latestClassification
      : "unknown",
    lowestPrice: price(value.lowestPrice),
    highestPrice: price(value.highestPrice),
    normalLowPrice: price(value.normalLowPrice),
    normalHighPrice: price(value.normalHighPrice),
    referencePrice: price(value.referencePrice),
    referenceObservedAt: timestamp(value.referenceObservedAt),
    referencePriceChangedAt: timestamp(value.referencePriceChangedAt),
    previousPrice: price(value.previousPrice),
    changeAmount: Number.isFinite(change) && Math.abs(change) <= 1_000_000
      ? Math.round(change * 100) / 100
      : null,
    trend: ["up", "down", "steady", "unknown"].includes(value.trend) ? value.trend : "unknown"
  };
}
...
function toAutomationProduct(product = {}) {
  ...
  delete automationProduct.imageUrl;
  ...
  delete automationProduct.sourceProvider;
  delete automationProduct.sourceProductId;
  delete automationProduct.sourceUrl;
  delete automationProduct.sourceListingId;
  delete automationProduct.sourceListingUrl;
  delete automationProduct.expectedPrice;
  delete automationProduct.expectedPriceConfidence;
  delete automationProduct.expectedPriceObservedAt;
  delete automationProduct.sourcePriceSummary;
...
function toRendererProduct(product = {}) {
  ...
  rendererProduct.imageUrl = String(product.imageUrl || "");
  ...
  Object.assign(rendererProduct, normalizeSourceFields(product));
...
function applyCheckoutPreflight(products = [], productIdValue, evidenceInput) {
  ...
  if (index < 0) throw new Error("That checkout store option is no longer configured.");
  const product = products[index];
  if (!product.enabled || product.action !== "checkout") {
    throw new Error("Checkout preflight requires an enabled auto-submit store option.");
  }
...
// Account-level checkout trust: the destination and payment fingerprints the
// operator approved once (per store and fulfillment mode) during any mission
// preflight. Auto-submit missions without their own preflight must match
// these; item, quantity, cart scope, and totals are still verified live.
const TRUST_FULFILLMENT_MODES = Object.freeze(["shipping", "pickup"]);

function normalizeCheckoutTrust(input, existing = {}) {
  const source = input && typeof input === "object" && !Array.isArray(input) ? input : existing;
  const trust = {};
  for (const retailer of Object.keys(RETAILERS)) {
    for (const mode of TRUST_FULFILLMENT_MODES) {
      const entry = source?.[retailer]?.[mode];
      if (!entry || typeof entry !== "object") continue;
      const destinationFingerprint = String(entry.destinationFingerprint || "");
      const pickupStoreFingerprint = String(entry.pickupStoreFingerprint || "");
      const instrumentSetFingerprint = String(entry.instrumentSetFingerprint || "");
      const instrumentCount = Number(entry.instrumentCount);
      const capturedAtDate = new Date(entry.capturedAt || "");
      if (
        !EVIDENCE_HASH_PATTERN.test(instrumentSetFingerprint)
        || !Number.isInteger(instrumentCount)
        || instrumentCount < 1
        || instrumentCount > 20
        || Number.isNaN(capturedAtDate.getTime())
        || (mode === "shipping" && (!EVIDENCE_HASH_PATTERN.test(destinationFingerprint) || pickupStoreFingerprint !== ""))
        || (mode === "pickup" && (!EVIDENCE_HASH_PATTERN.test(pickupStoreFingerprint) || destinationFingerprint !== ""))
      ) continue;
      trust[retailer] ||= {};
      trust[retailer][mode] = {
        ...
        instrumentCount,
        capturedAt: capturedAtDate.toISOString(),
        sourceProductId: cleanText(entry.sourceProductId, 80)
      };
    }
  }
  return trust;
...
// Combined-order gate: with the setting on, auto-submit missions of one
// store checkout together. Every eligible member must have a resolved status
// this run (carted, completed, or observed-and-not-in-progress) before the
// batch is ready; the captain — the first carted member by id — then drives
// one collective cart verification, review, and submission. Missions left out
// (out of stock during the pass) keep watching and simply form the next
// batch. Amazon is excluded: its cart and buy-box flows are not combined.
const COMBINED_ORDER_STORES = Object.freeze(["target", "walmart"]);

function combinedOrderStatus(settings, productStatuses = {}, calendarOwned = () => false) {
  if (!settings?.combinedOrderEnabled) return { enabled: false, stores: {} };
  const stores = {};
  for (const retailer of COMBINED_ORDER_STORES) {
    const members = (settings.products || []).filter((product) => (
      product.enabled
      && product.retailer === retailer
      && product.action === "checkout"
      && !calendarOwned(product)
    ));
    if (!members.length) continue;
    const entries = members.map((product) => {
      const status = productStatuses[product.id] || {};
      const completed = status.order === "confirmed";
      const carted = !completed && status.cart === "confirmed";
      const observed = Boolean(status.lastEventAt);
      const inProgress = !carted && !completed
        && (status.availability === "available" || ["adding", "added"].includes(status.cart));
      return {
        id: product.id,
        sku: product.sku,
        title: product.title || "",
        ...
        carted,
        completed,
        resolved: carted || completed || (observed && !inProgress)
      };
    });
    const active = entries.filter((entry) => !entry.completed);
    const carted = active.filter((entry) => entry.carted).sort((a, b) => a.id.localeCompare(b.id));
    const ready = active.length > 0 && active.every((entry) => entry.resolved) && carted.length >= 1;
    stores[retailer] = {
      ready,
      captainProductId: ready ? carted[0].id : "",
      members: entries.map(({ id, sku, title, quantity, maxPrice, maxOrderTotal, carted: isCarted }) => (
        { id, sku, title, quantity, maxPrice, maxOrderTotal, carted: isCarted }
      )),
      cartedCount: carted.length,
      pendingCount: active.filter((entry) => !entry.resolved).length,
      combinedMaxTotal: Math.round(carted.reduce((total, entry) => total + entry.maxOrderTotal, 0) * 100) / 100
    };
  }
  return { enabled: true, stores };
}

function checkoutTrustWithEvidence(trust, product, evidence) {
  const next = normalizeCheckoutTrust(trust || {}, {});
  if (!evidence?.fulfillment?.mode) return next;
  next[product.retailer] = {
    ...(next[product.retailer] || {}),
    [evidence.fulfillment.mode]: {
      destinationFingerprint: evidence.fulfillment.destinationFingerprint,
      pickupStoreFingerprint: evidence.fulfillment.pickupStoreFingerprint,
      instrumentSetFingerprint: evidence.payment.instrumentSetFingerprint,
      instrumentCount: evidence.payment.instrumentCount,
...
function normalizeProduct(input = {}) {
  ...
  if (signalEntry.startsWith("amazon-") && retailer !== "amazon") {
    throw new Error("Amazon direct signal entries require an Amazon store option.");
  }
  if (signalEntry === "walmart-buy-now" && retailer !== "walmart") {
    throw new Error("Walmart Buy Now signal entry requires a Walmart store option.");
  }
  if (signalEntry === "amazon-atc" && action === "watch") {
    throw new Error("Amazon Add to Cart signal entry requires an Add to cart or checkout store option.");
  }
  if (signalEntry === "amazon-buy-now" && !["review", "checkout"].includes(action)) {
    throw new Error("Amazon Buy Now signal entry requires a checkout-review or auto-submit store option.");
  }
  if (signalEntry === "walmart-buy-now" && !["review", "checkout"].includes(action)) {
    throw new Error("Walmart Buy Now signal entry requires a checkout-review or auto-submit store option.");
  }
  ...
  const sourceFields = normalizeSourceFields(input);

  const product = {
    itemId: itemIdForProduct({ ...input, id: productId(retailer, sku), retailer, sku }),
    id: productId(retailer, sku),
    retailer,
    title: cleanText(input.title, 80),
    imageUrl: normalizeProductImageUrl(input.imageUrl, retailer),
    openAt,
    productUrl,
    sku,
...
function normalizeProducts(
  ...
  // An empty item plan is legal; arming still requires an enabled store option.
  ...
  const ids = new Set();
  const itemStoreKeys = new Set();
  const itemIds = new Set();
  const itemSharedValues = new Map();
  for (const product of products) {
    ...
    const itemStoreKey = `${product.itemId}:${product.retailer}`;
    if (itemStoreKeys.has(itemStoreKey)) {
      throw new Error(`An item can have only one ${retailerLabel(product.retailer)} store option.`);
    }
    itemStoreKeys.add(itemStoreKey);
    const sharedValues = JSON.stringify(Object.fromEntries(SHARED_FIELDS.map((field) => [field, product[field]])));
    if (itemSharedValues.has(product.itemId) && itemSharedValues.get(product.itemId) !== sharedValues) {
      throw new Error("Store options for one item must share the same name, quantity, schedule, action, fulfillment, alerts, profile, group, and signal behavior.");
    }
    itemSharedValues.set(product.itemId, sharedValues);
  }
  if (itemIds.size > MAX_ITEMS) throw new Error(`An item plan can contain at most ${MAX_ITEMS} items.`);
  return products;
}
...
function normalizeSettings(input = {}, existing = {}) {
  ...
  const trackalackerSignalDedupeWindowSeconds = normalizeWholeSeconds(
    input.trackalackerSignalDedupeWindowSeconds
      ?? existing.trackalackerSignalDedupeWindowSeconds
      ?? DEFAULT_SETTINGS.trackalackerSignalDedupeWindowSeconds,
    MIN_SIGNAL_DEDUPE_WINDOW_SECONDS,
    MAX_SIGNAL_DEDUPE_WINDOW_SECONDS,
    "Signal dedupe window"
  );
  const trackalackerSignalAliases = normalizeTrackalackerSignalAliases(
    input.trackalackerSignalAliases
      ?? existing.trackalackerSignalAliases
      ?? DEFAULT_SETTINGS.trackalackerSignalAliases
  );
  const signalStrategies = normalizeSignalStrategies(
    input.signalStrategies
      ?? existing.signalStrategies
      ?? DEFAULT_SETTINGS.signalStrategies
  );
  const trackalackerSignalBridgeEnabled = input.trackalackerSignalBridgeEnabled !== undefined
    ? Boolean(input.trackalackerSignalBridgeEnabled)
    : Boolean(existing.trackalackerSignalBridgeEnabled ?? DEFAULT_SETTINGS.trackalackerSignalBridgeEnabled);
  const trackalackerSignalDeliveryPaused = input.trackalackerSignalDeliveryPaused !== undefined
    ? Boolean(input.trackalackerSignalDeliveryPaused)
    : Boolean(existing.trackalackerSignalDeliveryPaused ?? DEFAULT_SETTINGS.trackalackerSignalDeliveryPaused);
  ...
  const signalsEnabled = input.signalsEnabled !== undefined
    ? Boolean(input.signalsEnabled)
    : Boolean(existing.signalsEnabled ?? DEFAULT_SETTINGS.signalsEnabled);
  if (automationEnabled && signalsEnabled) {
    throw new Error("Choose either Autopilot or Signals, not both.");
  }
  ...
  if (automationEnabled || signalsEnabled) {
    ...
    const authorizedProducts = signalsEnabled
      ? enabledProducts
      : [...enabledProducts, ...walmartPrepCandidates];
    const trackalackerCanSeedMissions = signalsEnabled
      && trackalackerSignalBridgeEnabled
      && !trackalackerSignalDeliveryPaused;
    if (!authorizedProducts.length && !trackalackerCanSeedMissions) {
      throw new Error(signalsEnabled
        ? "Enable at least one product or connect unpaused TrackaLacker Push before starting Signals."
        : "Enable at least one product or add a Walmart prep candidate before arming automation.");
    }
    const priceAuthorizedProducts = authorizedProducts.filter((product) => product.action !== "watch");
    if (priceAuthorizedProducts.some((product) => product.maxPrice <= 0)) {
      throw new Error("Every enabled purchase product needs a positive maximum unit price before automation can be armed.");
    }
  ...
  return {
    products,
    checkoutTrust: normalizeCheckoutTrust(input.checkoutTrust, existing.checkoutTrust ?? DEFAULT_SETTINGS.checkoutTrust),
    missionGroups,
    automationEnabled,
    signalsEnabled,
    monitoringPaused,
    ...
    combinedOrderEnabled: input.combinedOrderEnabled !== undefined
      ? Boolean(input.combinedOrderEnabled)
      : Boolean(existing.combinedOrderEnabled ?? DEFAULT_SETTINGS.combinedOrderEnabled),
    ...
    trackalackerSignalBridgeEnabled,
    trackalackerSignalDeliveryPaused,
    trackalackerSignalDedupeWindowSeconds,
...
function purchaseConfigFingerprint(settings = {}) {
  ...
  return JSON.stringify({
    ...
    signalStrategies: normalizeSignalStrategies(settings.signalStrategies || [])
  });
}

function assertSafeArmedUpdate(current, next) {
  if (
    purchaseModeEnabled(current)
    && purchaseModeEnabled(next)
    && purchaseConfigFingerprint(current) !== purchaseConfigFingerprint(next)
  ) {
    throw new Error("Disarm automation (Autopilot or Signals) before changing products, signal strategies, quantities, price caps, or purchase actions.");
  }
}

function purchaseModeEnabled(settings = {}) {
  return Boolean(settings.automationEnabled || settings.signalsEnabled);
}
...
function validateEvent(input) {
  ...
  const imageUrl = normalizeProductImageUrl(input.imageUrl, retailer);
...
module.exports = {
  ...
  MAX_TRACKALACKER_SIGNAL_ALIASES,
  ...
  newCompanionToken,
  normalizeProduct,
  normalizeCheckoutTrust,
  checkoutTrustWithEvidence,
  combinedOrderStatus,
  ...
  normalizeTrackalackerSignalAliases,
  normalizeTargetUrl,
