...
const ACTION_MODES = new Set(["cart", "review", "checkout"]);
...
function normalizeProduct(input = {}) {
  ...
  if (!ACTION_MODES.has(action)) {
    throw new Error("Choose Add to cart only, Stop at final review, or Submit order automatically.");
  }
...
function normalizeSettings(input = {}, existing = {}) {
  ...
  if (automationEnabled) {
    ...
    const purchaseProducts = enabledProducts.filter((product) => product.action !== "cart");
...
function purchaseConfigFingerprint(settings = {}) {
  return JSON.stringify((settings.products || []).map((product) => ({
    id: product.id,
