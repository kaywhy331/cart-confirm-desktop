...
function normalizeWalmartPrepCandidates(input = [], existingProducts = []) {
  ...
  for (const value of source) {
    const product = normalizeProduct({ ...value, enabled: true });
    if (
...
function normalizeProducts(input, fallback, missionGroups = []) {
...
function normalizeSettings(input = {}, existing = {}) {
  ...
  const products = normalizeProducts(inputProducts, existingProducts, missionGroups);
