...
function normalizeProductForSettings(value, storeOrderAllowances) {
  ...
  product.maxOrderTotal = calculateOrderTotalCap(product, storeOrderAllowances);
  product.checkoutEvidence = normalizeCheckoutEvidence(value?.checkoutEvidence, product);
  return product;
}

function normalizeProducts(input, fallback, missionGroups = [], storeOrderAllowances = DEFAULT_STORE_ORDER_ALLOWANCES) {
  ...
  const products = rawProducts.map((value) => {
    const product = normalizeProductForSettings(value, storeOrderAllowances);
...
function normalizeSettings(input = {}, existing = {}) {
  ...
  const products = normalizeProducts(inputProducts, existingProducts, missionGroups, storeOrderAllowances);
  ...
  if (automationEnabled) {
    ...
    if (authorizedProducts.some((product) => product.action === "checkout" && !product.checkoutEvidence)) {
      throw new Error("Every auto-submit product needs an approved checkout preflight before Autopilot can be armed.");
    }
  }
