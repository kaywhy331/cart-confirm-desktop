...
function normalizeSettings(input = {}, existing = {}) {
  ...
  const retryIntervalSeconds = Number(
    input.retryIntervalSeconds
      ...
      ?? DEFAULT_SETTINGS.retryIntervalSeconds
  );
  if (
    !Number.isInteger(retryIntervalSeconds)
    || retryIntervalSeconds < MIN_RETRY_SECONDS
    || retryIntervalSeconds > MAX_RETRY_SECONDS
  ) {
    throw new Error(`Retry interval must be a whole number from ${MIN_RETRY_SECONDS} to ${MAX_RETRY_SECONDS} seconds.`);
  }
...
function eventMessage(event) {
  ...
  switch (event.eventType) {
    ...
    case "order-submit-clicked":
      return `${store} order submission was selected for the armed product.`;
