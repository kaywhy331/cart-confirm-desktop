...
const MAX_RESULTS_PER_CAPTURE = 25;
...
function catalogMissionFromItem(item) {
...
function planCatalogMissionImport(state, selectedIds, existingProducts = [], maximum = MAX_PRODUCTS) {
  ...
  for (const id of requested) {
    ...
    const mission = catalogMissionFromItem(item);
    existingIds.add(mission.id);
    additions.push(mission);
  }
  return {
    additions,
    summary: { selected: requested.length, imported: additions.length, duplicates, missing, overCapacity }
  };
}
