...
function createStoreOpenQueue(options = {}) {
  ...
  async function enqueue(retailer, action, options = {}) {
    ...
    // A bounded scheduled-drop burst may deliberately launch dedicated tabs
    // together. It still observes Stop, cooldown, and the caller's action
    // budget, but it does not wait behind another browser handoff.
    if (options.parallel === true) return run();
