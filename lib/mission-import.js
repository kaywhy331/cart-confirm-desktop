...
function bulkMissionFromUrl(value, defaults = {}) {
  ...
  return defaults.profile
    ? normalizeProduct(applyItemProfile(
        inertMission,
        defaults.profile,
...
function planBulkImport(text, existingProducts = [], maximum = MAX_PRODUCTS, defaults = {}) {
  ...
  for (const entry of entries) {
    ...
    let product;
    try {
      product = bulkMissionFromUrl(entry.url, defaults);
    } catch {
      invalid += 1;
  ...
  return {
    ...
    summary: {
      ...
      ready: additions.filter((product) => product.enabled).length,
      needsPrice: additions.filter((product) => !product.enabled).length,
      duplicates,
      invalid,
      overCapacity
    }
...
function quickAddMission(input = {}, defaults = {}) {
  ...
  // A capture from a tracking-decorated page must never fail the quick add;
  // the canonical product link alone is still a complete mission.
  let affiliateOpenUrl = "";
  try {
    affiliateOpenUrl = normalizeAffiliateOpenUrl(input.affiliateOpenUrl, retailer, urlSku);
  } catch {
    affiliateOpenUrl = "";
  }

  const observedMission = normalizeProduct({
    retailer,
  ...
  return defaults.profile
    ? normalizeProduct(applyItemProfile(
        ...
        defaults.profile,
        defaults.msrpCatalog || [],
        // Quick add is an explicit capture of the exact open product page.
        // Its observed price is the promised cap even when the title also
        // matches a broader approved MSRP category.
        {
