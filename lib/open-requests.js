...
function createOpenRequestStore(options = {}) {
  ...
  function add(retailer, url) {
    ...
    return { id, retailer: request.retailer, url: request.url };
  }
  ...
  function claim(id) {
    ...
    request.settle(true);
    return { ok: true, id: request.id, retailer: request.retailer, url: request.url };
  }
  ...
  return Object.freeze({ add, pending, claim, cancel, waitForClaim });
}
