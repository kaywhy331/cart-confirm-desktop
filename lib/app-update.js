...
function signedAppxReleasePlan(release) {
  if (!release || release.draft || release.prerelease || !Array.isArray(release.assets)) return null;
  const version = parseVersion(release.tag_name);
  if (!version || String(release.tag_name) !== `v${version.text}`) return null;
  const appxName = `Cart-Confirm-Signals-${version.text}-x64.appx`;
  const appx = release.assets.find((asset) => asset?.name === appxName);
  const checksums = release.assets.find((asset) => asset?.name === "SHA256SUMS.txt");
  if (!appx?.browser_download_url || !checksums?.browser_download_url) return null;
  return {
    version: version.text,
    tagName: `v${version.text}`,
    releaseName: String(release.name || `Cart Confirm v${version.text}`).slice(0, 160),
    publishedAt: String(release.published_at || ""),
    pageUrl: `${RELEASES_PAGE_URL}/tag/v${version.text}`,
    appxAsset: {
      name: appxName,
      size: Number(appx.size || 0),
      url: assertReleaseAssetUrl(appx.browser_download_url)
    },
    checksumsAsset: {
      name: "SHA256SUMS.txt",
      size: Number(checksums.size || 0),
...
function selectSignedAppx(releases, currentVersion) {
  const current = parseVersion(currentVersion);
  if (!current) throw new Error("The installed app version is invalid.");
  const plans = (Array.isArray(releases) ? releases : [])
    .map(signedAppxReleasePlan)
    .filter(Boolean)
    // A same-version stable AppX is the expected migration path from an
    // unsigned prerelease. Never offer a signed package older than the
    // currently running desktop build.
    .filter((plan) => compareVersions(plan.version, current) >= 0)
...
async function checkForSignedAppx(currentVersion) {
  return selectSignedAppx(await fetchReleases(currentVersion), currentVersion);
}

async function downloadVerifiedReleaseAsset(plan, asset, destinationDirectory, onProgress, label) {
  assertReleaseAssetUrl(asset.url);
  assertReleaseAssetUrl(plan.checksumsAsset.url);
  if (asset.size > MAX_INSTALLER_BYTES) throw new Error(`The ${label} is unexpectedly large.`);
  ...
  const expectedHash = parseChecksumManifest(checksumBody.toString("utf8"), asset.name);

  fs.mkdirSync(destinationDirectory, { recursive: true, mode: 0o700 });
  const destination = path.join(destinationDirectory, asset.name);
  const temporary = `${destination}.download`;
  fs.rmSync(temporary, { force: true });
  const hash = crypto.createHash("sha256");
  let received = 0;
  try {
    const response = await openResponse(asset.url, {
      allowedHosts: DOWNLOAD_HOSTS,
    ...
    const total = asset.size || responseLength;
    const meter = new Transform({
      transform(chunk, _encoding, callback) {
        received += chunk.length;
        if (received > MAX_INSTALLER_BYTES) {
          callback(new Error(`The ${label} exceeded the maximum allowed size.`));
          return;
        }
        hash.update(chunk);
    ...
    if (asset.size && received !== asset.size) {
      throw new Error(`The downloaded ${label} size did not match the GitHub release asset.`);
    }
    onProgress({ phase: "verifying", received, total });
    const actualHash = hash.digest("hex");
    if (actualHash !== expectedHash) {
      throw new Error(`The downloaded ${label} failed SHA-256 verification and will not be opened.`);
    }
    fs.rmSync(destination, { force: true });
    fs.renameSync(temporary, destination);
    return { assetPath: destination, sha256: actualHash, size: received };
  } catch (error) {
    fs.rmSync(temporary, { force: true });
    throw error;
  }
...
async function downloadUpdate(plan, destinationDirectory, onProgress = () => {}) {
  ...
  const result = await downloadVerifiedReleaseAsset(
    plan,
    plan.setupAsset,
  ...
  return { installerPath: result.assetPath, sha256: result.sha256, size: result.size };
}

async function downloadSignedAppx(plan, destinationDirectory, onProgress = () => {}) {
  if (!plan?.appxAsset || !plan?.checksumsAsset) throw new Error("The selected signed AppX release is incomplete.");
  const result = await downloadVerifiedReleaseAsset(
    plan,
    plan.appxAsset,
    destinationDirectory,
  ...
  return { packagePath: result.assetPath, sha256: result.sha256, size: result.size };
}
