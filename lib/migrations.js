...
function migrateStoredSettings(input) {
  ...
  const products = (Array.isArray(input.products) ? input.products : []).map((product) => {
    ...
    if (Number(product.maxOrderTotal) === maxOrderTotal && product.itemId === itemId) return product;
    return {
      ...product,
      itemId,
      maxOrderTotal,
  ...
  const purchaseModeEnabled = Boolean(input.automationEnabled || input.signalsEnabled);
  const unsafeCheckout = purchaseModeEnabled && (unsafeProductCheckout || unsafeLegacyCheckout);
  const conflictingModes = Boolean(input.automationEnabled && input.signalsEnabled);
  ...
  let next = {
    ...
    ...(unsafeCheckout || conflictingModes ? {
