"use strict";

const { productCalendarOwned } = require("./schedule");

const QUEUE_FANOUT_SPACING_MS = 0;
const QUEUE_FANOUT_DRAIN_BUFFER_MS = 3_000;
const QUEUE_FANOUT_MAX_DRAIN_MS = 60_000;
const SUPPORTED_RETAILERS = new Set(["walmart"]);

function queueFanoutKey(runId, retailer, cohortId) {
  const normalizedRunId = String(runId || "").trim();
  const normalizedRetailer = String(retailer || "").toLowerCase();
  const normalizedCohortId = String(cohortId || "").trim();
  if (!normalizedRunId || !normalizedCohortId || !SUPPORTED_RETAILERS.has(normalizedRetailer)) return "";
  return `${normalizedRunId}|${normalizedRetailer}|${normalizedCohortId}`;
}

function planQueueFanout(options = {}) {
  const settings = options.settings || {};
  const event = options.event || {};
  if (
    !settings.automationEnabled
    || settings.monitoringPaused
    || event.eventType !== "queue-waiting"
    || event.availability === "unavailable"
  ) {
    return null;
  }

  const capture = options.capture;
  const retailer = String(event.retailer || "").toLowerCase();
  const key = queueFanoutKey(settings.automationRunId, retailer, capture?.cohortId);
  if (!key || options.receipts?.[key]) return null;
  if (
    capture?.retailer !== retailer
    || capture.runId !== String(settings.automationRunId || "")
    || !Array.isArray(capture.participantProductIds)
    || !capture.participantProductIds.includes(String(event.productId || ""))
  ) return null;
  const sourceProduct = (settings.products || []).find((product) => product?.id === event.productId);
  if (sourceProduct && productCalendarOwned(settings, sourceProduct)) return null;

  const excluded = new Set([
    String(event.productId || ""),
    ...(options.queuedProductIds || []).map(String)
  ]);
  const productIds = (settings.products || [])
    .filter((product) => (
      product
      && product.enabled
      && product.retailer === retailer
      && capture.participantProductIds.includes(String(product.id || ""))
      && !productCalendarOwned(settings, product)
      && !excluded.has(String(product.id || ""))
    ))
    .map((product) => product.id);

  if (!productIds.length) return null;
  const openRequestDrainMs = Math.min(
    QUEUE_FANOUT_MAX_DRAIN_MS,
    productIds.length * QUEUE_FANOUT_SPACING_MS + QUEUE_FANOUT_DRAIN_BUFFER_MS
  );
  return Object.freeze({
    key,
    retailer,
    productIds,
    spacingMs: QUEUE_FANOUT_SPACING_MS,
    parallel: true,
    dedicatedTab: true,
    openRequestDrainMs
  });
}

module.exports = {
  QUEUE_FANOUT_SPACING_MS,
  planQueueFanout,
  queueFanoutKey
};
