...
const SIGNAL_PRICE_BAND_RANK = Object.freeze({
  below_msrp: 0,
  msrp: 1,
  slightly_above_msrp: 2,
  above_msrp: 3
});
...
function normalizeSignalStrategies(input = []) {
  ...
  for (let index = 0; index < input.length; index += 1) {
    ...
    strategies.push({
      ...
      action,
      quantity,
      allowThirdPartySeller: candidate.allowThirdPartySeller === true,
      includeKeywords,
      excludeKeywords
    });
  }
...
function priceBandWithinCap(priceBand, capBand) {
  const cap = cleanText(capBand || "any", 40).toLowerCase();
  if (cap === "any") return true;
  const actualRank = SIGNAL_PRICE_BAND_RANK[priceBand];
  const capRank = SIGNAL_PRICE_BAND_RANK[cap];
  return actualRank !== undefined && capRank !== undefined && actualRank <= capRank;
}
...
function matchSignalStrategy(options = {}) {
  ...
  for (let index = 0; index < strategies.length; index += 1) {
    ...
    if (!priceBandWithinCap(priceBand, strategy.priceBand)) continue;
