...
(function initItemDefaults(globalScope) {
  ...
  const DEFAULT_ITEM_PROFILE_ID = "built-in:shipping-watch";
  const DEFAULT_ORDER_TAX_PERCENT = 12;
  const DEFAULT_STORE_ORDER_ALLOWANCES = Object.freeze({
    target: 30,
    walmart: 30,
    amazon: 30
  });
  ...
  function normalizeStoreOrderAllowances(input, fallback = DEFAULT_STORE_ORDER_ALLOWANCES) {
    const source = input && typeof input === "object" && !Array.isArray(input) ? input : {};
    const backup = fallback && typeof fallback === "object" && !Array.isArray(fallback)
      ? fallback
      : DEFAULT_STORE_ORDER_ALLOWANCES;
    const labels = { target: "Target", walmart: "Walmart", amazon: "Amazon" };
    return Object.fromEntries(RETAILERS.map((retailer) => {
      const allowance = Number(
        source[retailer]
          ?? backup[retailer]
          ?? DEFAULT_STORE_ORDER_ALLOWANCES[retailer]
      );
      if (!Number.isFinite(allowance) || allowance < 0 || allowance > 1_000_000) {
        throw new Error(`${labels[retailer]} order-total allowance must be from $0.00 to $1,000,000.00.`);
      }
      return [retailer, roundMoney(allowance)];
    }));
  }

  function normalizeOrderTaxPercent(value, fallback = DEFAULT_ORDER_TAX_PERCENT) {
    const percent = Number(value ?? fallback);
    if (!Number.isFinite(percent) || percent < 0 || percent > 100) {
      throw new Error("Order tax percentage must be from 0% to 100%.");
    }
    return roundMoney(percent);
  }

  function calculateOrderTotalCap(
    product = {},
    allowances = DEFAULT_STORE_ORDER_ALLOWANCES,
    orderTaxPercent = DEFAULT_ORDER_TAX_PERCENT
  ) {
    if (!["review", "checkout"].includes(String(product.action || ""))) return 0;
    const maxPrice = Number(product.maxPrice);
    const quantity = Number(product.quantity);
    if (!Number.isFinite(maxPrice) || maxPrice <= 0 || !Number.isInteger(quantity) || quantity < 1) return 0;
    const normalizedAllowances = normalizeStoreOrderAllowances(allowances);
    const normalizedTaxPercent = normalizeOrderTaxPercent(orderTaxPercent);
    const allowance = normalizedAllowances[String(product.retailer || "").toLocaleLowerCase()] || 0;
    const subtotal = maxPrice * quantity;
    return roundMoney(subtotal + (subtotal * normalizedTaxPercent / 100) + allowance);
  }
    ...
    return {
      ...
      maxOrderTotal: needsOrderTotal && hasApprovedCap
        ? calculateOrderTotalCap(
            applied,
            options.storeOrderAllowances,
            options.orderTaxPercent
          )
        : 0
    };
  }
