...
const { extractSku, parseRetailUrl } = require("./retailers");

const MAX_QUIET_HTML_BYTES = 8 * 1024 * 1024;
...
function quietMonitorError(code, message) {
  const error = new Error(message);
  error.code = code;
  return error;
}

function assertQuietProductResponse(responseUrl, retailer, sku) {
  let parsed;
  let detectedRetailer;
  try {
    ({ parsed, retailer: detectedRetailer } = parseRetailUrl(responseUrl));
  } catch {
    throw quietMonitorError("redirect-mismatch", "The quiet check left the approved retailer.");
  }
  const routeAllowed = retailer === "target"
    ? /^\/p\//i.test(parsed.pathname)
    : retailer === "walmart"
  ...
  if (
    ...
    || !routeAllowed
    || extractSku(retailer, parsed.href) !== String(sku || "")
  ) {
    throw quietMonitorError("redirect-mismatch", "The quiet check did not end on the exact canonical product page.");
  }
  return parsed.href;
}

async function readBoundedHtml(response, options = {}) {
  const maximumBytes = Number(options.maximumBytes ?? MAX_QUIET_HTML_BYTES);
  if (!Number.isInteger(maximumBytes) || maximumBytes < 1) {
    throw quietMonitorError("invalid-body-limit", "The quiet response limit is invalid.");
  }
  const contentType = String(response?.headers?.get?.("content-type") || "").toLowerCase();
  if (!/^(?:text\/html|application\/xhtml\+xml)(?:\s*;|$)/.test(contentType)) {
    throw quietMonitorError("invalid-content-type", "The quiet response was not an HTML product page.");
  }
  const contentLength = Number(response.headers.get("content-length"));
  if (Number.isFinite(contentLength) && contentLength > maximumBytes) {
    throw quietMonitorError("body-too-large", "The quiet product page exceeded the response limit.");
  }
  const reader = response?.body?.getReader?.();
  if (!reader) throw quietMonitorError("unreadable-body", "The quiet product page had no readable response body.");
  const chunks = [];
  let total = 0;
  try {
    while (true) {
...
function looksLikeSecurityChallenge(html) {
  const visibleText = String(html || "")
    ...
    .replace(/<[^>]+>/g, " ")
    .replace(/\s+/g, " ");
  return /captcha|robot check|verify (?:that )?you(?:'|’)re human|verify you are (?:a )?human|press and hold|unusual traffic|security challenge|access denied/i.test(visibleText);
}
