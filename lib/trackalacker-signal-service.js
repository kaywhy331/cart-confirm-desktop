...
function baseRecord(parsed, resolution, keys, receivedAt, timing) {
  ...
  return {
    ...
    retailer: mapping.retailer || parsed.retailer,
...
function processTrackalackerSignal(options = {}) {
  ...
  if (parsed.envelope.testSignal || parsed.testNotification) {
    ...
    record.reason = resolution.matchMethod === "store-identity-missing"
      ? "TrackaLacker identified a preorder but did not expose an exact listing or retailer SKU; no store mission was selected."
      : hasSafeIdentity
        ...
        : parsed.retailer
          ...
          : "The live TrackaLacker alert did not expose enough safe product and store identity; no mission action was allowed.";
