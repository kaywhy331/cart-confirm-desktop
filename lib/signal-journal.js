...
function normalizeSignalRecord(input = {}) {
  ...
  return {
    ...
    strategyId: cleanText(input.strategyId, 80),
    strategyName: cleanText(input.strategyName, 80),
    strategyAction: ["notify", "add_to_cart", "prepare_checkout", "submit_order"].includes(input.strategyAction)
      ? input.strategyAction
      : "",
    strategyQuantity: input.strategyQuantity === "max"
      ? "max"
      : Number.isInteger(Number(input.strategyQuantity)) && Number(input.strategyQuantity) >= 1 && Number(input.strategyQuantity) <= 5
        ? Number(input.strategyQuantity)
        : "",
    missionCreated: input.missionCreated === true,
    missionDecision,
    actionState,
...
function duplicateSemanticRecord(state, input = {}, windowSeconds = 300) {
  ...
  return (state?.records || []).find((record) => {
    // A live page can invalidate a once-correct signal without invalidating
    // the mission. A new transport after that terminal offer mismatch is a
    // legitimate re-signal, even inside the normal semantic dedupe window.
    if (record.missionDecision === "offer_mismatch" && record.actionState === "cancelled") return false;
    // Corroboration is coalesced by the active product activation, not used as
    // a new semantic lock after that activation later ends.
    if (record.missionDecision === "corroborated") return false;
