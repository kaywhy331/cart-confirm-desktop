...
6. Switch **Autopilot** on. If the companion is not already connected, the app opens one mission page in Chrome and waits for the connection automatically. Remaining unscheduled Target and Walmart missions begin quiet background checks and open after a likely stock signal; Amazon remains tab-based. Scheduled missions stay gated until their configured time even if one is used as the harmless connection page. The app always restarts fully stopped and never resumes a saved run without this explicit action.
