...
- A Chrome-toolbar **Quick add** popup that reads the exact TCIN / Walmart item ID / ASIN, retailer title, and current price from the open product page. The positive observed price becomes the cap; the default item profile fills quantity, shipping, action, and alert, while the retailer setting supplies the final-total allowance. Existing duplicates are never changed, and a missing/non-positive price is refused.
...
3. The app calculates the maximum final order total from `unit cap × quantity + that retailer's saved allowance`. You can still choose **Watch & alert only**, **Add to cart only**, **Prepare checkout, I submit**, or a different fulfillment mode for this mission.
...
2. Set one extra order-total allowance for each retailer to cover expected tax, shipping, and fees. Then create or update named item profiles for quantity, action, fulfillment, alert, and enabled preference. Choose the default profile used by Quick add, URL import, new missions, and Catalog Inbox.
