...
- A dense mission-control layout: each product is a mission card showing its compact price/quantity, action, live status, and last-checked age in one row, with exact caps retained in tooltips and accessibility text. A validated retailer-hosted product image captured by Quick add or the first observed product page appears as a small thumbnail before the store name; hovering the mission row or focusing its thumbnail opens a medium preview. Use drag-and-drop or the accessible up/down arrows to reorder missions. Search and filter by named group, retailer, or On/Off state; group headers can collapse, rename, or turn every member On/Off, and group assignment is available in both the mission editor and plan tools.
...
- Up to 100 unique products across Target, Walmart, and Amazon, each with an optional display name.
...
- A focused **Edit plan** session in Missions. It pauses an active run once, adds selection controls directly to the filtered mission rows, and groups profile, On/Off, group, shared-time, clear-time, and copy-list actions beside the list. Individual mission saves do not repeatedly restart automation; **Finish editing** confirms and starts the updated run once. Changing a mission's profile applies it immediately, and updating a reusable profile offers to refresh every linked mission in the same change.
- A compact **Run readiness** card that summarizes Chrome connection, enabled and scheduled mission counts, action mix, blocking conditions, and maximum exposure before a run. Catalog and Item defaults start collapsed, remember their state, and remain directly reachable from Missions.
...
3. The app calculates the maximum final order total from `item subtotal + configured tax + that retailer's saved allowance`. The global tax estimate defaults to 12% and each store allowance defaults to $30; adjust both under **Item defaults** for your location and expected shipping or fees. You can still choose **Watch & alert only**, **Add to cart only**, **Prepare checkout, I submit**, or a different fulfillment mode for this mission.
4. For auto-submit, a positive unit cap, sufficient positive final-total cap, and explicit shipping/pickup requirement are mandatory. Advance checkout preflight is optional so sold-out or unreleased missions can arm. When stock appears, the live final-review page must still prove the destination/store, complete payment set, disabled substitutions, exact single cart line, quantity, SKU, first-party offer, fulfillment, and capped total; that evidence is checked again immediately before the click. If you optionally lock a preflight from the companion popup, any mismatch stops submission.
5. Optionally enter an **Affiliate product link**. It must be a direct HTTPS retailer URL for this mission’s exact store and item ID. Only that mission’s explicit **Open** button prefers it (then a validated admin campaign link, then the canonical product link); watchers, schedules, signals, and purchase automation always use the canonical product link. Arbitrary short links are rejected.
6. Optionally choose a named mission group, set **Open at** for a known drop time, pick an alert loudness, and choose how a matched Discord signal enters the store under **Advanced**. The product page is the recommended default. Direct Buy Now entries are available only for checkout-review or auto-submit missions; Amazon Add to Cart is available for cart or checkout missions.
...
1. Choose **Edit plan** and create the complete mission list. If Autopilot was running, it pauses once; save as many individual or bulk changes as needed, then choose **Finish editing** once.
2. Empty unrelated items from any store cart that will use checkout mode.
3. Use **Monitor only** once to open every enabled mission without a calendar time through the paced queue, and confirm on each row that the page, price, and seller are recognized. Nothing is added while Autopilot is off; scheduled missions wait for their configured time.
...
6. Switch **Autopilot** on. If the companion is not already connected, the app opens one mission page in Chrome and waits for the connection automatically. Remaining unscheduled Target and Walmart missions begin quiet background checks and open after a likely stock signal, or right after their first check when the public page proves browser-only; Amazon remains tab-based. Scheduled missions stay gated until their configured time even if one is used as the harmless connection page. The app always restarts fully stopped and never resumes a saved run without this explicit action.
...
8. Watch the mission rows and the Activity feed (click a row to filter it). Activity stores only operator milestones: **Watch Started**, **Qualified**, **Added to Cart**, **Ordered**, and **Notified**. Page scans, unavailable observations, retry scheduling, navigation, and internal status chatter remain visible on the mission/status surfaces when relevant but do not populate Activity. **Stop everything** is always available.
