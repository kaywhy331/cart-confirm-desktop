...
- Before an offer becomes eligible, waiting missions share a rapid per-store refresh lane (two seconds by default). As soon as a mission verifies an eligible offer, that mission leaves the rapid lane; cart and checkout recovery continue using the normal traffic spacing.
- All desktop and extension store actions, including rapid eligibility refreshes, share a fixed 120-action rolling-hour budget. Each product is limited to 100 purchase attempts in a four-hour run and requires manual re-arming after exhaustion.
...
- Configurable pre-eligibility refresh spacing from 2 to 60 seconds (two seconds by default), isolated from the slower cart and checkout retry cadence.
...
6. Switch **Autopilot** on. It immediately opens every enabled mission without a calendar time; scheduled missions stay closed and open exactly at their configured time. The app always restarts fully stopped and never resumes a saved run without this explicit action.
...
The desktop opening queue and extension share the same safety goal: enter known drops promptly without turning normal monitoring into an unbounded reload storm. Manual and test openings use a fixed three-second stagger per store. Scheduled drop openings and the one exactly-once official-queue fan-out use a one-second stagger, one page load per mission. Once a tab reports that it is queued, Cart Confirm stops refreshing it. While product-page offers remain ineligible, those tabs share the configurable pre-eligibility lane (two seconds per store by default); with six same-store missions, a complete waiting sweep is therefore about 12 seconds plus page-load time instead of roughly two minutes at the normal 20-second spacing. An eligible mission immediately leaves that lane. Cart, checkout, watch-mode, and error retries stay on the normal configured per-store spacing. A product's fresh reservation replaces its own stale one so no product can starve the others, and main-frame navigations are observed so a retry cannot immediately follow a newly opened page. Identical retry notices collapse in Activity while each mission's last-checked time continues updating. Every opening and retry still consumes the same fixed 120-action rolling-hour budget.
