...
- Every desktop launch starts fully **STOPPED**: Autopilot is disarmed, browser scans and retries are paused, background checks and schedules cannot fire, and Discord is not polled. Starting an edit while armed pauses the run once and opens a plan-edit session; individual saves stay paused until **Finish editing** deliberately starts one updated run.
...
- Catalog listing prices and AI research are untrusted hints. TrackaLacker imports are also not live purchase proof: only a same-store history classified by TrackaLacker as normal/MSRP-like can make that imported store option ready after the operator reviews the preview; surge, above-MSRP, missing-history, and product-level fallback estimates remain Off for cap review. The signed-in retailer page is still authoritative before any action. Cited research is never applied to MSRP or existing missions automatically.
...
- A persistent run-state banner that distinguishes **Stopped**, **Monitoring only**, and **Autopilot active**. **Review & start** opens a structured preflight with item counts, scheduled work, action mix, maximum exposure, blockers, and warnings before the user chooses monitoring or Autopilot.
...
- A product-first **TrackaLacker followed-products** importer. The companion uses the already signed-in TrackaLacker browser session—never a password supplied to Cart Confirm—to read bounded followed pages and product/listing history sequentially. The preview shows one product with its thumbnail, TrackaLacker source, supported Target/Walmart/Amazon store toggles, exact store routes, history links, expected prices, confidence, and future-store labels. Selected store options import under one shared item ID instead of becoming item+store duplicates.
...
Each listing’s visible history table is summarized locally. Entries labeled **Price Surge**, **Above MSRP**, or described as higher/scalper pricing are excluded. The most stable remaining same/MSRP-like price becomes that store’s expected cap and can use the chosen item profile immediately. If only the followed-product display estimate exists, Cart Confirm retains it as a convenient proposed cap but leaves that store option Off for review. Import never changes existing routes, and the 100-store-option plan limit still applies.

The scan uses only the browser’s existing TrackaLacker cookies. Cart Confirm does not ask for, transmit, log, or persist TrackaLacker credentials. Product preview data is stored locally in `trackalacker-import.json`; it contains normalized product/listing identifiers, source/history URLs, image URL, expected-price summaries, status, and timestamps—not cookies or passwords.
...
3. Choose **Edit plan** for multi-row changes. If Autopilot was running, it pauses once; use **Undo** or **Revert plan** if needed, then choose **Finish editing** once.
...
5. Use add-only mode for an initial low-risk test, then reopen **Review & start** when you are ready for Autopilot. The app always restarts fully stopped and never resumes a saved run without this explicit approval.
